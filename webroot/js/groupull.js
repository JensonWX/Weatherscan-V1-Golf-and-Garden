...
function marqueeComplete() {
	$('#marquee2').fadeOut(0); 
	$('#marquee2-shadow').fadeOut(0); 
	$('#marqueenext').fadeIn(0);
}
function switchMarquee2(idx) {
	...
	$('#marquee2')
		...
		.on('finished', function() {
			marqueeComplete()
			setTimeout(function(){
				switchMarquee2(((idx < apperanceSettings.marqueeAd.length) ? ++idx : 0))
			},15000)
		});
...
function MarqueeMan() {
	function switchToWarningMarquee(sidx) {
		if (weatherInfo.bulletin.severeweathermode == true) {
			...
			$('.marqueeheadersevere').text((weatherInfo.bulletin.severewarnings[sidx].warningname) + ' ' + weatherInfo.bulletin.severewarnings[sidx].warningstatus).replaceAll('Severe Thunderstorm Warning Update', 'Severe T\'Storm Warning Update');
			$('#arrow-img').fadeOut(0)
