...
function Loops() {
	...
	refreshObservati()
	//setInterval(refreshObservati,60000);
}
function refreshObservati() {
	...
	function displayAtmospheric(idx) {
		...
		var displayCity = 1;
		if(!weatherInfo.currentCond.weatherLocs[1]){
			displayCity = 0;
		}
		var displays = {
			currently() {
				if (weatherInfo.currentCond.sidebar.noReport == false) {
					return 'Currently at <b>' + weatherInfo.currentCond.weatherLocs[displayCity].displayname + '<b>'
				} else {
					return ''
				}
			},
			conditions() {
				if (weatherInfo.currentCond.sidebar.noReport == false) {
					return 'Currently <b>' + weatherInfo.currentCond.weatherLocs[displayCity].cond + '<b>'
				} else {
					return ''
				}
			},
			temp(){ 
				if (weatherInfo.currentCond.sidebar.noReport == false) {
					return 'Currently <b>' + weatherInfo.currentCond.weatherLocs[displayCity].temp + '°<b>'
				} else {
					return ''
				}
			},
			heatindex(){
				if (weatherInfo.currentCond.sidebar.noReport == false) {
					if (weatherInfo.currentCond.weatherLocs[displayCity].feelslike.type != "dontdisplay") {
						return 'Current ' + weatherInfo.currentCond.weatherLocs[displayCity].feelslike.type + ' <b>' + weatherInfo.currentCond.weatherLocs[displayCity].feelslike.val + '°<b>'
					}
				} else {
					return ''
				}
